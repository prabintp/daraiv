...
export class AuthTokenService implements CanActivate {
    ...
    init(options?: Angular2TokenOptions) {
        ...
        let defaultOptions: Angular2TokenOptions = {
            ...
            authBase:                   false,//sign in auth type
    ...
    /*    let body = JSON.stringify({
        ...
        let observ = this.post(this.getUserPath() + this.atOptions.signInPath, JSON.stringify(signInData));
    ...
    validateToken(): Observable<Response> {
        ...
        let observ = this.get(this.getUserPath() + this.atOptions.validateTokenPath +'?access_token='+this.atCurrentAuthData.accessToken);
    ...
    request(options: RequestOptionsArgs): Observable<Response> {
        ...
        if(this.atOptions.authBase){
          let baseParam = JSON.parse(options.body);
    ...
    private getAuthHeadersFromResponse(data: any): void {
        ...
        let shop = result.user.shops.find(function(item){if(item.isactive == true) return true});
        let shops = result.user.shops;
